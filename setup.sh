...
virsh list --all | grep running | awk '{print $2}' | while read vm_name; do
    ...
    execute_with_retry "resolvectl flush-caches; ssh-keyscan -H $vm_name >> ~/.ssh/known_hosts" "resolvectl flush-caches; ssh -o IdentitiesOnly=yes -i ~/.ssh/allowed_keys -n $SSH_USER@$vm_name 'echo $vm_name'"

    execute_with_retry \
        "ssh -o IdentitiesOnly=yes -i ~/.ssh/allowed_keys -n $SSH_USER@$vm_name 'until [ -f /home/$SSH_USER/fin ]; do sleep 1; done'" \
        "ssh -o IdentitiesOnly=yes -i ~/.ssh/allowed_keys -n $SSH_USER@$vm_name 'echo /home/$SSH_USER/fin'"
