...
virsh list --all | grep running | awk '{print $2}' | while read vm_name; do
    ...
    execute_with_retry "resolvectl flush-caches; ssh-keyscan -H $vm_name >> ~/.ssh/known_hosts" "resolvectl flush-caches; ssh -n -i ~/.ssh/id_rsa.pub $SSH_USER@$vm_name 'echo $vm_name'"
